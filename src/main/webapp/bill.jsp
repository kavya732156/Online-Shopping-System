...
try
{
	...
	ResultSet rs2=stmt.executeQuery("select * from users inner join cart where cart.email='"+email+"' and cart.status='bill' ");
	while(rs2.next())
	{
		
%>
<h3>Online shopping Billing part</h3>
<hr>
...
<h3>Product Details</h3>
  ...
  <% 
  ResultSet rs4=stmt.executeQuery("select * from cart inner join product where cart.product_id=product.id and cart.email='"+email+"' and cart.status='bill'");
  while(rs4.next())
  {
	 sno=sno+1;
  %>
  <tr>
    ...
    <td><%=rs4.getString(17) %></td>
    <td><%=rs4.getString(18) %></td>
    <td><%=rs4.getString(19) %></td>
